...
function useGameEngine(startDiff=1,seed=null){
  ...
  const finishRound=useCallback(ok=>{clearInterval(noiseRef.current);if(!ok)setShowCorrect(true);setHolsterPhase("cooldown");setTimeout(()=>{setRoundNum(p=>p+1);setHolsterPhase("idle");setArmProgress(0);setShowCorrect(false);},ok?1000:2000);},[]);
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden",minHeight:0}}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",background:C.bg,position:"relative"}} onMouseEnter={g.handleHolsterEnter} onMouseLeave={g.handleHolsterLeave}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"14px 16px 12px",gap:14,flexShrink:0,borderBottom:`1px solid ${C.border}`,cursor:"crosshair",background:g.holsterPhase==="armed"?`radial-gradient(ellipse at center,rgba(72,187,120,0.06) 0%,transparent 70%)`:g.holsterPhase==="arming"?`radial-gradient(ellipse at center,rgba(236,201,75,0.04) 0%,transparent 70%)`:"none",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{textAlign:"left"}}><div style={{fontSize:10,fontWeight:700,letterSpacing:2.5,color:g.holsterPhase==="armed"?C.green:g.holsterPhase==="arming"?C.yellow:g.holsterPhase==="cooldown"?C.red:g.holsterPhase==="live"?`${C.green}70`:C.textGhost,textTransform:"uppercase",transition:"color 0.25s",textShadow:g.holsterPhase==="armed"?`0 0 10px ${C.green}30`:"none"}}>{g.holsterPhase==="armed"?"ARMED — GO":g.holsterPhase==="arming"?`ARMING ${Math.round(g.armProgress*100)}%`:g.holsterPhase==="cooldown"?"COOLDOWN":g.holsterPhase==="live"?"ROUND LIVE":"HOVER TO ARM"}</div>{g.holsterPhase==="idle"&&<div style={{fontSize:8.5,color:C.textGhost,marginTop:3}}>Rest mouse here for 0.8s</div>}</div>
          </div>
...
function OneVOneMode(){
  ...
  async function storageSet(k,v,shared=true){try{await window.storage.set(k,JSON.stringify(v),shared);}catch(e){}}
  async function storageGet(k,shared=true){try{const r=await window.storage.get(k,shared);return r?JSON.parse(r.value):null;}catch{return null;}}
...
const CSS=`
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .menu-bg{min-height:100%;height:100%;background:${C.bg};display:flex;flex-direction:column;align-items:center;justify-content:center;
