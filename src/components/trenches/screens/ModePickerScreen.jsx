...
function ModePickerScreen({session,onSelect,onLogOut}){
  ...
  const cards=[
    {key:"practice",label:"SOLO_DRILL",desc:"Sharpen identification speed and accuracy in a controlled environment.",icon:"⚡",color:C.green},
    {key:"1v1",label:"ARENA_DUEL",desc:"Direct head-to-head combat with synchronized signal feeds.",icon:"⚔",color:C.orange},
    {key:"profile",label:"INTEL_STATS",desc:"Deep dive into operational metrics and performance history.",icon:"📊",color:C.cyan},
  ];
    ...
    <div className="menu-bg prac-page"><div className="grid-bg"/>
      <div className="prac-shell" style={{maxWidth:900,width:"100%"}}>
        ...
        <div style={{textAlign:"center",marginBottom:48}}>
          <div style={{fontSize:10,color:C.green,letterSpacing:5,fontWeight:800,marginBottom:16}}>// WELCOME_BACK_AGENT</div>
          <h1 style={{fontSize:56,fontWeight:900,letterSpacing:-3,color:C.text,marginBottom:12}}>{username.toUpperCase()}</h1>
          <div style={{fontSize:12,color:C.textMuted,maxWidth:500,margin:"0 auto"}}>System ready. Select operational mode to begin.</div>
        </div>
        ...
        <div style={{textAlign:"center"}}>
          <button onClick={onLogOut} className="btn-ghost" style={{fontSize:9,padding:"8px 24px"}}>TERMINATE_SESSION</button>
        </div>
